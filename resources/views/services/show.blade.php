...
@section('content')
    <div class="jumbotron jumbotron-fluid"
        ...
        <div class="container margin-top-50">
            ...
            <h4 class="display-6 text-center">{{ $city->city }}, {{ $city->state_code }}</h4>
            @elseif(isset($location) && !is_null($location))
            <h4 class="display-6 text-center">{{ $location->city->city }}, {{ $location->city->state_code }}</h4>
            @endif
            <div class="col-sm-6 mx-auto" style="background-color: rgb(255, 255, 255);color: #000;">
                <div class="p-sm-5 pt-5 pb-5">
                    ...
                    <div class="input-group mb-3">
                        <input id="city_id_input" value="{{ $location->city_id == $city->id ?$location->zip:$location->zip }}" type="text" class="form-control" placeholder="Enter your zip code" aria-label="Enter your zip code" aria-describedby="go-button">
            ...
            <div class="modal fade service-question-modal questions-modal" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <form action="{{url('/services/'.$service->id)}}" method="POST" enctype="multipart/form-data" id="questionsForm">
                        @csrf
                        ...
                        <div class="modal-body">
                            <input id="city_id" type="hidden" name="city_id" value="{{$location->city_id == $city->id ? $location->zip : $city->areas()->first()->zip}}" />
                            ...
                            @foreach($service->questions as $question)
                                ...
                                @endforeach
                                @continue($continue)
                                <div class="service-question service-question-{{$question->type}}" id="service-question-{{ $question->id }}"
                                        @if($i != 1) style="display: none;" @endif data-required="{{$required}}" data-type="{{$question->type}}">
                                    <h3>{{$question->question}}{{ $required ? '*' : '' }} </h3>
                                    @if($question->type == \App\Models\ServiceQuestion::TYPE_BOOLEAN)
                                        <div class="form-check">
                                            <input name="answer-{{$question->id}}" class="form-check-input" type="radio" id="answer-boolean-{{$question->id}}-yes" value="1" @if($required) required @endif @if(old('answer-'.$question->id, false)) checked @endif>
                                            <label class="form-check-label" for="answer-boolean-{{$question->id}}-yes">
                                                Yes
                                            </label>
                                        </div>
                                        <div class="form-check">
                                            <input name="answer-{{$question->id}}" class="form-check-input" type="radio" id="answer-boolean-{{$question->id}}-no" value="0" @if($required) required @endif @unless(old('answer-'.$question->id, false)) checked @endunless>
                                            <label class="form-check-label" for="answer-boolean-{{$question->id}}-no">
                                                No
                                            </label>
                                        </div>
                                    @elseif($question->type == \App\Models\ServiceQuestion::TYPE_TEXT)
                                        @if($question->name == 'guest.email')
                                            <input type="email" name="answer-{{$question->id}}" class="form-control" @if($required) required @endif value="{{old('answer-'.$question->id)}}" />
                                        @elseif($question->name == 'guest.phone')
                                            <input type="tel" name="answer-{{$question->id}}" class="form-control" @if($required) required @endif value="{{old('answer-'.$question->id)}}" />
                                        @else
                                            <input type="text" name="answer-{{$question->id}}" class="form-control" @if($required) required @endif value="{{old('answer-'.$question->id)}}" />
                                        @endif
                                    @elseif($question->type == \App\Models\ServiceQuestion::TYPE_TEXT_MULTILINE)
                                        <textarea name="answer-{{$question->id}}" class="form-control" @if($required) required @endif>{{old('answer-'.$question->id)}}</textarea>
                                    @elseif($question->type == \App\Models\ServiceQuestion::TYPE_DATE)
                                        <input type="date" name="answer-{{$question->id}}" class="form-control" @if($required) required @endif value="{{old('answer-'.$question->id)}}" />
                                    @elseif($question->type == \App\Models\ServiceQuestion::TYPE_TIME)
                                        <input type="time" name="answer-{{$question->id}}" class="form-control" @if($required) required @endif value="{{old('answer-'.$question->id)}}" />
                                    @elseif($question->type == \App\Models\ServiceQuestion::TYPE_DATE_TIME)
                                        <input type="datetime-local" name="answer-{{$question->id}}" class="form-control" @if($required) required @endif value="{{old('answer-'.$question->id)}}" />
                                    @elseif($question->type == \App\Models\ServiceQuestion::TYPE_FILE)
                                        <input type="file" name="answer-{{$question->id}}" @if($required) required @endif accept="image/*" />
                                    @elseif($question->type == \App\Models\ServiceQuestion::TYPE_FILE_MULTIPLE)
                                        <input type="file" name="answer-{{$question->id}}[]" @if($required) required @endif accept="image/*" multiple />
                                    @elseif($question->type == \App\Models\ServiceQuestion::TYPE_SELECT)
                                        @foreach($question->choices as $choice)
                                            <div class="form-check">
                                                <input name="answer-{{$question->id}}" class="form-check-input" type="radio" id="answer-choice-{{$choice->id}}" value="{{$choice->id}}" @if($required) required @endif @if(old('answer-'.$question->id, false) == $choice->id) checked @endif>
                                    ...
                                    @elseif($question->type == \App\Models\ServiceQuestion::TYPE_SELECT_MULTIPLE)
                                        @foreach($question->choices as $choice)
                                            <div class="form-check">
                                                <input name="answer-{{$question->id}}[]" class="form-check-input" type="checkbox" id="answer-choice-{{$choice->id}}" value="{{$choice->id}}" @if($required) required @endif @if( in_array($choice->id, old('answer-'.$question->id, [])) ) checked @endif>
                                                <label class="form-check-label" for="answer-choice-{{$choice->id}}">
                                                    {{$choice->choice}}
                                                </label>
                                            </div>
